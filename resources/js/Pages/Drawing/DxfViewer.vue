...
/* ── stato ── */
const fileName = ref('')
const drawables = ref([])                 // line | poly | circle | text | mtext | hatchPath
const layersMap = ref({})
const stylesMap = ref({})
...
/* tweak regolabili da UI per i TEXT allineati Top/Bottom (percentuale dell’altezza) */
const topTweak = ref(-0.28)     // sposta in SU i TEXT “Top”
const bottomTweak = ref(+0.08)  // sposta in GIÙ i TEXT “Bottom”
...
/* ── costanti ── */
const LINE_SCALE = 2
const FILL_RETINO   = '#ADD8E6' // celeste
const FILL_MANIGLIA = '#000000' // nero
...
async function onFile(e){
  ...
  let dxf; try{ dxf=new DxfParser().parseSync(text) }catch(err){ alert('DXF non valido: '+err.message); return }
  ...
  // fill a livello layer (path unico per evenodd)
  const fills=[]
...
<template>
  <Head title="DXF Viewer — sfondi retino+maniglia, testi corretti" />
  <div class="p-6 space-y-4">
    <h1 class="text-xl font-semibold">DXF Viewer (sfondo "retino" + "maniglia")</h1>

    <div class="flex flex-wrap items-center gap-4">
      <input type="file" accept=".dxf" @change="onFile" class="border p-2 rounded" />
      <label class="text-sm"><input type="checkbox" v-model="showFill" /> Mostra sfondi</label>
    ...
    <!-- Riquadro fisso 200×300, sfondo trasparente -->
